...
## 28. sum을 이용해서 2차원 배열 -> 1차원 배열로 변환하기
<code>arr = [[1],[2,3],[4,5,6]]</code> 이라는 배열을 [1,2,3,4,5,6]으로 변경하고 싶을 때가 있다.
sum() 함수를 Python 공식문서에서 깊게 파보면 sum(iterable, start=0)을 발견할 수 있다. start 시작 인자를 함께 더할 수 있는 것인데,
예를들어 <code>a = [1,2,3]; sum(a,-1)</code>을 실행하면 <code> -1 + 1 + 2 + 3 = 5</code>가 나오는 것이다.
...
sum(arr,[])에서는 다음과 같은 식이 수행된다. <code>[] + [1] + [2,3] + [4,5,6]</code>
